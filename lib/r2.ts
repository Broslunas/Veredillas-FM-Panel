...
export type R2UploadTarget = 'auto' | 'image' | 'audio' | 'video' | 'clip' | 'social';
...
export async function resolveUploadDestination(
  ...
  if (target === 'image' || (target === 'auto' && isImage)) {
    ...
    resolvedType = 'social';
    resolvedFolder = resolvedFolder || 'social-clips';
...
export async function uploadFileToR2(
  ...
  if (currentUsage.totalBytes + fileBuffer.length > bucket.maxBytes) {
    await notifyUploadBlocked(bucket, fileBuffer.length, currentUsage.totalBytes, fileName);
